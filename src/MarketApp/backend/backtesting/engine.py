...
Purpose
-------
Run expanding-window walk-forward validation for the threshold-direction XGBoost
model.

Why it matters
--------------
Walk-forward validation is one of DiMarket's strongest trust signals because it
evaluates the model in a time-aware way instead of relying on random splits.
"""
...
def run_walk_forward_engine(
    ...
    while start + config.test_size <= len(X):
        ...
        future_returns_test = feats[
            "future_return"
        ].iloc[
            train_end:test_end
        ]

        future_dates_test = feats[
            "date"
        ].iloc[
            train_end:test_end
        ]
        ...
        all_future_returns.extend(
            future_returns_test.tolist()
        )

        all_future_dates.extend(
            future_dates_test.tolist()
        )
        ...
        top10 = top_bucket_win_rate(
            probs,
            y_test,
            0.10,
        )

        top20 = top_bucket_win_rate(
            probs,
            y_test,
            0.20,
        )

        top30 = top_bucket_win_rate(
            probs,
            y_test,
            0.30,
        )
