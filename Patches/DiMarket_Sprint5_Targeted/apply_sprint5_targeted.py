...
def main() -> None:
    ...
    if "const [forecastWarning, setForecastWarning]" in dashboard:
        raise RuntimeError(
            "Sprint 5 appears to already be present. No files were changed."
        )
...
const PLAN_CARDS = [
  ...
  },
  {
    ...
    state_old = "  const [showForecastReminder, setShowForecastReminder] = useState(false)"
    state_new = (
        state_old
        + "\n  const [forecastWarning, setForecastWarning] = useState(null)"
    )
    dashboard = replace_exact_once(
        dashboard,
        state_old,
        state_new,
        "Forecast warning state",
    )
...
"""
    dashboard = replace_exact_once(
        ...
        prediction_marker,
        banner,
        "Extended-horizon result banner",
    )

    current_renderer = r"""{PLAN_CARDS.map(([key, name, tagline, features]) => (
            <article
              key={key}
    ...
    new_renderer = r"""{PLAN_CARDS.map((plan) => (
            <article
              key={plan.key}
...
"""
    ...
    # Only now create backups and write, after every validation succeeded.
    ...
    print()
    print("Next: restart Vite and verify the Plans section.")
    print("Stripe billing still requires a new Gold price at $24.99/month.")
